...
import { subgraphUrl, subgraphUrlPermanent, validChains } from 'config/tcr-addresses'
...
const createGraphQLClient = (
  ...
  if (!url) return null
  if (!clients[key]) {
    clients[key] = new GraphQLClient(url)
  }
  return clients[key]
}
...
export const getPermanentGraphQLClient = (
  chainId: string | number
): GraphQLClient | null => createGraphQLClient(chainId, subgraphUrlPermanent, 'permanent-')
