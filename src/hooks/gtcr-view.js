...
const useGtcrView = () => {
  ...
  if (process.env.REACT_APP_GTCRVIEW_ADDRESSES)
    try {
      arbitrableTCR2ViewData = JSON.parse(
        process.env.REACT_APP_GTCRVIEW_ADDRESSES
      )[web3Context.networkId || DEFAULT_NETWORK]
    } catch (_) {
      console.error('Failed to parse env variable REACT_APP_GTCRVIEW_ADDRESSES')
    }
